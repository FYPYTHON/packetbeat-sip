			...
			for _, header_s := range values.([]common.NetString) {
				...
				if mode, ok := sip.parseSet[key]; ok {
					...
					case SIP_DETAIL_NAME_ADDR:
						...
						if display_name != "" {
							newobj["display"] = display_name
						}
						if user_info != "" {
							newobj["user"] = user_info
						}
						if host != "" {
							newobj["host"] = host
						}
...
func (sip *sipPlugin) ReceivedFin(tcptuple *common.TCPTuple, dir uint8,
        private protos.ProtocolData) protos.ProtocolData {
        return private
}

func (sip *sipPlugin) GapInStream(tcptuple *common.TCPTuple, dir uint8,
